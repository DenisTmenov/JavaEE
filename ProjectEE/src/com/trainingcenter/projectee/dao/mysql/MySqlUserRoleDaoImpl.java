...
public class MySqlUserRoleDaoImpl implements UserRoleDao {
	...
	/*
	 * private UserRoleBean createUserRoleBean(ResultSet set) throws
	 * SQLException { Integer idRole = set.getInt("id_role"); String nameRole =
	 * set.getString("name_role"); Boolean add = set.getBoolean("add"); Boolean
	 * delete = set.getBoolean("delete"); Boolean modify =
	 * set.getBoolean("modify"); Boolean read = set.getBoolean("read");
	 * 
	 * UserRoleBean bean = new UserRoleBean();
	 * 
	 * bean.setIdRole(idRole); bean.setNameRole(nameRole); bean.setAdd(add);
	 * bean.setDelete(delete); bean.setModify(modify); bean.setRead(read);
	 * 
	 * return bean; }
	 */
}
